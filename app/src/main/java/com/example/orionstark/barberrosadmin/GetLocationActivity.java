    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        myMap.setOnMyLocationChangeListener(new GoogleMap.OnMyLocationChangeListener() {
            ...
            public void onMyLocationChange(Location location) {
                ...
                try {
                    List<Address> listAddress = geocoder.getFromLocation(location.getLatitude(),
                            location.getLongitude(), 1);
                    if ( null != listAddress && listAddress.size() > 0 ) {
                        start_location = new LatLng(location.getLatitude(), location.getLongitude());
                    }
                } catch (Exception e) {
                    Toast.makeText(getApplicationContext(), "We can't get the place. Please change the direction",
            ...
            case 0x3:
                switch (resultCode) {
                    ...
                    case Activity.RESULT_CANCELED:
                        /* So, you didn't want me to access your location?, ok try me B :) */
